...
export default function Navbar() {
  ...
  return (
    <header className="flex items-center justify-between p-3 lg:p-6">
      {/* Logo */}
      ...
      <div className="flex items-center gap-5">
        ...
        <nav className="hidden lg:flex items-center space-x-6 relative z-50">
          <NavLink
            to="/"
            className={({ isActive }) => `luxury-nav text-[#D89F30] hover:text-white transition-colors duration-300 pb-0.5 font-semibold text-xs lg:text-sm tracking-wide ${isActive ? 'border-b-2 border-[#D89F30]' : 'border-b-2 border-transparent hover:border-[#D89F30]/60'}`}
            end
          >
            HOME
          </NavLink>
          <span className="text-[#D89F30] text-sm lg:text-base font-bold">|</span>
          <NavLink
            to="/about"
            className={({ isActive }) => `luxury-nav text-[#D89F30] hover:text-white transition-colors duration-300 pb-0.5 font-semibold text-xs lg:text-sm tracking-wide ${isActive ? 'border-b-2 border-[#D89F30]' : 'border-b-2 border-transparent hover:border-[#D89F30]/60'}`}
          >
          ...
          >
            <button type="button" className="luxury-nav text-[#D89F30] hover:text-white transition-colors duration-300 font-semibold text-xs lg:text-sm tracking-wide flex items-center gap-2">
              <span>SERVICES</span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-h-[70vh] overflow-y-auto pr-2">
                  {servicesData.map((cat) => (
                    <div key={cat.title} className="">
                      <h4 className="luxury-nav text-[#D89F30] text-sm lg:text-base font-semibold tracking-wide">
                        {cat.title}
                      </h4>
                      {cat.tagline ? (
                      ...
                      <div className="space-y-4">
                        {cat.subcategories?.map((sub) => (
                          <div key={sub.name}>
                            <h5 className="text-white font-semibold text-xs lg:text-sm mb-1">{sub.name}</h5>
          ...
          <span className="text-[#D89F30] text-sm lg:text-base font-bold">|</span>
          <a href="#" className="luxury-nav text-[#D89F30] hover:text-white transition-colors duration-300 font-semibold text-xs lg:text-sm tracking-wide">
            GALLERY
          </a>
          <span className="text-[#D89F30] text-sm lg:text-base font-bold">|</span>
          <NavLink
        ...
        {/* Mobile Menu Button */}
        <button className="lg:hidden text-[#D89F30] p-2" aria-label="Open menu">
